...
if ($checkDb->num_rows > 0) {
    echo "<p id='suc'>Database <strong>$dbname</strong> already exists ✅</p>";
} else {
    // Attempt to create database if it doesn't exist
    $sql = "CREATE DATABASE $dbname";
    if ($conn->query($sql) === TRUE) {
        echo "<p id='suc'>Database <strong>$dbname</strong> created successfully ✅</p>";
    } else {
        echo "<p id='err'>Error creating database: " . $conn->error . "</p>";
    }
}
